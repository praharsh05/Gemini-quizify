...
class DocumentProcessor:
    ...
    def ingest_documents(self):
        ...
        2. For each uploaded PDF file:
           a. Generate a unique identifier and append it to the original file name before saving it
             temporarily. This avoids name conflicts and maintains traceability of the file.
        ...
        if uploaded_files is not None:
            for uploaded_file in uploaded_files:
                # Step 2 (a): Generate a unique identifier to append to the file's original name
                unique_id = uuid.uuid4().hex
                ...
                # Step 2 (b): Process the temporary file
                # Use PyPDFLoader here to load the PDF and extract pages.
                # https://python.langchain.com/docs/modules/data_connection/document_loaders/pdf#using-pypdf

                loader = PyPDFLoader(temp_file_path)
                pages = loader.load()  # This will extract the pages from the PDF

                # Step 3: Then, Add the extracted pages to the 'pages' list.
                self.pages.extend(pages)

                # Step 2 (c): Clean up by deleting the temporary file.
